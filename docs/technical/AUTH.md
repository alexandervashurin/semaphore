...
**Примечание:** Токен добавляется в чёрный список (реализовано через `TokenBlacklist` с DashMap in-memory, автоочистка каждые 5 минут).
...
3. **Используйте чёрный список** - для отзыва токенов (реализовано в `TokenBlacklist`)
...
| Хранение сессий | JWT + TokenBlacklist (DashMap in-memory) |
| Bcrypt cost | 10 | 12 |
...
- [x] Чёрный список токенов (TokenBlacklist, DashMap, автоочистка каждые 5 мин)
- [x] Refresh токены (30 дней TTL)
- [x] 2FA (TOTP RFC 6238, recovery codes)
- [x] OIDC / OAuth2 провайдеры (auto-discovery, email_claim)
- [x] LDAP аутентификация с синхронизацией групп
- [x] Rate limiting для login
- [x] Аудит успешных/неуспешных входов (Events)
