...
- ✅ 6 встроенных типов webhook (Generic, Slack, Teams, Discord, Telegram, Custom)
...
Если webhook настроен с `secret`, при отправке вычисляются HMAC-SHA256 подпись:

**Заголовки запроса:**
```
Content-Type: application/json
X-Semaphore-Signature: sha256=<hmac_signature>
X-Semaphore-Timestamp: <unix_timestamp>
```

Подпись вычисляется как `HMAC-SHA256(secret, timestamp + body)`. Получатель может верифицировать подпись, пересчитав HMAC с тем же секретом.

> ⚠️ **Важно:** Заголовок `Authorization: Bearer` **НЕ используется**. Вместо него — HMAC-подпись в `X-Semaphore-Signature`.
...
- **Формула**: `100ms * 2^(attempt-1)` (экспоненциальная задержка, без верхнего лимита)
