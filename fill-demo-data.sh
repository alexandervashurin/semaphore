#!/bin/bash
# ============================================================================
# fill-demo-data.sh — заполняет демо-данными запущенный Velum
# Запуск: bash fill-demo-data.sh
# Требует: curl, jq
# ============================================================================
...
INV=$(call -X POST "$BASE/project/$PID/inventory" \
  -d "{\"name\":\"ansible-target\",\"project_id\":$PID,\"type\":\"static\",
    \"inventory\":\"[targets]\nansible-target ansible_host=172.19.0.2 ansible_port=22 ansible_user=demo ansible_ssh_pass=demo123 ansible_become=true ansible_become_method=sudo ansible_become_pass=demo123\",
    \"ssh_key_id\":$KID}")
...
REPO=$(call -X POST "$BASE/project/$PID/repositories" \
  -d "{\"name\":\"demo-playbooks\",\"project_id\":$PID,\"git_url\":\"file:///app/playbooks\",\"git_branch\":\"master\",\"ssh_key_id\":$KID}")
...
echo ""
echo "✅ Готово! Откройте http://localhost:8088"
