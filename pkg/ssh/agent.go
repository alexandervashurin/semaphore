...
func StartSSHAgent(key db.AccessKey, logger task_logger.Logger) (Agent, error) {
	// Генерация случайной строки для имени сокета
	randomBytes := make([]byte, 10)
	rand.Read(randomBytes)
	randomString := hex.EncodeToString(randomBytes)[:10]

	socketFilename := fmt.Sprintf("ssh-agent-%d-%s.sock", key.ID, randomString)
