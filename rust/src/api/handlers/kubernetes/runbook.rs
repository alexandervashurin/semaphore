...
pub async fn get_available_runbooks(
    ...
    let runbooks = templates
        ...
        .map(|tpl| {
            ...
            } else if tpl.name.contains("fix")
                || tpl.name.contains("remediate")
                || tpl.name.contains("restore")
            {
...
pub async fn execute_runbook(
    ...
    let template = state
        .store
        .get_template(project_id, payload.template_id)
        .await
        .map_err(|e| match e {
    ...
    let task = Task {
        ...
        message: payload.message.or_else(|| {
            Some(format!(
...
fn build_runbook_arguments(payload: &RunbookRequest, playbook: &str) -> Option<String> {
...
mod tests {
    ...
    fn test_kubernetes_context_with_labels() {
        ...
        assert_eq!(
            ctx.labels.as_ref().unwrap().get("app"),
            Some(&"web".to_string())
        );
    }
