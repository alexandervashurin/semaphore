...
fn pv_summary(pv: &PersistentVolume) -> PersistentVolumeSummary {
    ...
    PersistentVolumeSummary {
        name: pv.metadata.name.clone().unwrap_or_else(|| "unknown".to_string()),
        status,
        capacity: spec
...
fn pvc_summary(pvc: &PersistentVolumeClaim) -> PersistentVolumeClaimSummary {
    ...
    PersistentVolumeClaimSummary {
        name: pvc.metadata.name.clone().unwrap_or_else(|| "unknown".to_string()),
        namespace: pvc
            .metadata
            .namespace
            .clone()
...
fn sc_summary(sc: &StorageClass) -> StorageClassSummary {
    let is_default = sc
        ...
        .unwrap_or(false);
    StorageClassSummary {
        name: sc.metadata.name.clone().unwrap_or_else(|| "unknown".to_string()),
...
pub async fn delete_persistent_volume(
    ...
    api.delete(&name, &DeleteParams::default())
        .await
        .map_err(|e| Error::Kubernetes(e.to_string()))?;
    Ok(Json(serde_json::json!({"status":"ok","message":format!("PersistentVolume {} deleted", name)})))
}
...
pub async fn delete_storage_class(
    ...
    api.delete(&name, &DeleteParams::default())
        .await
        .map_err(|e| Error::Kubernetes(e.to_string()))?;
    Ok(Json(serde_json::json!({"status":"ok","message":format!("StorageClass {} deleted", name)})))
}
