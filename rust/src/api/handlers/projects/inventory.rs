...
use crate::db::store::{InventoryManager, RetrieveQueryParams};
...
pub async fn get_inventories(
    ...
    let inventories = state.store.get_inventories(project_id).await.map_err(|e| {
        (
...
pub async fn add_inventory(
    ...
    let created = state.store.create_inventory(inventory).await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse::new(e.to_string())),
        )
