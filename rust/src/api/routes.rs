...
pub fn api_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/internal/runners/{id}/task",
            get(runners::runner_get_task),
        )
        .route(
            "/api/internal/tasks/{task_id}/log",
            post(runners::runner_submit_log),
        )
        ...
        .route(
            "/api/kubernetes/clusters/{cluster_id}/namespaces/{namespace}/pods/{name}/exec",
            get(handlers::pod_exec),
        )
        .route(
            "/api/kubernetes/clusters/{cluster_id}/namespaces/{namespace}/pods/{name}/portforward",
            get(handlers::pod_portforward),
        )
        ...
        .route(
            "/api/kubernetes/namespaces/{namespace}/daemonsets/{name}/restart",
            post(handlers::kubernetes::workloads_k8s::restart_daemonset),
        )
        .route(
        ...
        .route(
            ...
            put(handlers::scale_statefulset),
        )
        .route(
        ...
        .route(
            "/api/kubernetes/k8s-events",
            get(handlers::kubernetes::workloads_k8s::list_k8s_events),
        )
        // ── Phase 8: Observability ────────────────────────────────────────
        .route(
            "/api/kubernetes/metrics/status",
            get(handlers::kubernetes::observability::get_metrics_status),
        )
        .route(
        ...
        .route(
            "/api/kubernetes/topology/namespaces/{namespace}",
            get(handlers::kubernetes::observability::get_namespace_topology),
        )
        // ── Phase 9: Helm ─────────────────────────────────────────────────
        .route(
            "/api/kubernetes/helm/status",
            get(handlers::kubernetes::helm_handler::helm_status),
        )
        .route(
            "/api/kubernetes/helm/repos",
            get(handlers::list_helm_repos).post(handlers::add_helm_repo),
        )
        .route(
            "/api/kubernetes/helm/repos/update",
            post(handlers::kubernetes::helm_handler::update_helm_repos),
        )
        .route(
            "/api/kubernetes/helm/repos/{name}",
            delete(handlers::kubernetes::helm_handler::remove_helm_repo),
        )
        .route(
            "/api/kubernetes/helm/releases",
            get(handlers::list_helm_releases)
                .post(handlers::kubernetes::helm_handler::install_helm_release),
        )
        .route(
            "/api/kubernetes/helm/search",
            get(handlers::search_helm_charts),
        )
        .route(
            "/api/kubernetes/helm/namespaces/{ns}/releases/{name}",
            get(handlers::kubernetes::helm_handler::get_helm_release)
                .put(handlers::upgrade_helm_release)
                .delete(handlers::uninstall_helm_release),
        )
        .route(
            "/api/kubernetes/helm/namespaces/{ns}/releases/{name}/history",
            get(handlers::kubernetes::helm_handler::helm_release_history),
        )
        .route(
            "/api/kubernetes/helm/namespaces/{ns}/releases/{name}/rollback",
            post(handlers::rollback_helm_release),
        )
        .route(
            "/api/kubernetes/helm/namespaces/{ns}/releases/{name}/values",
            get(handlers::get_helm_release_values),
        )
        .route(
            "/api/kubernetes/helm/namespaces/{ns}/releases/{name}/manifest",
            get(handlers::kubernetes::helm_handler::get_helm_release_manifest),
        )
        .route(
            "/api/kubernetes/helm/charts/{chart}/values",
            get(handlers::kubernetes::helm_handler::get_chart_default_values),
        )
        ...
        .route(
            ...
            get(handlers::kubernetes::apply::generate_kubectl_command),
        )
        .route(
            "/api/kubernetes/clusters",
            get(handlers::list_configured_clusters),
        )
        .route(
            "/api/kubernetes/clusters/switch",
            post(handlers::switch_cluster_context),
        )
}
