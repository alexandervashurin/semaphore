...
use crate::models::{DeploymentEnvironment, DeploymentEnvironmentCreate, DeploymentEnvironmentUpdate, DeploymentRecord};
use async_trait::async_trait;
use sqlx::Row;

#[async_trait]
impl DeploymentEnvironmentManager for SqlStore {
    async fn get_deployment_environments(&self, project_id: i32) -> Result<Vec<DeploymentEnvironment>> {
        let rows = sqlx::query(
    ...
    async fn get_deployment_environment(&self, id: i32, project_id: i32) -> Result<DeploymentEnvironment> {
        let row = sqlx::query(
            "SELECT * FROM deployment_environment WHERE id = $1 AND project_id = $2"
        )
        .bind(id)
        .bind(project_id)
    ...
    async fn create_deployment_environment(&self, project_id: i32, payload: DeploymentEnvironmentCreate) -> Result<DeploymentEnvironment> {
        let row = sqlx::query(
    ...
    async fn update_deployment_environment(&self, id: i32, project_id: i32, payload: DeploymentEnvironmentUpdate) -> Result<DeploymentEnvironment> {
        let row = sqlx::query(
    ...
    async fn get_deployment_history(&self, env_id: i32, project_id: i32) -> Result<Vec<DeploymentRecord>> {
        let rows = sqlx::query(
        ...
        Ok(rows.iter().map(|r| DeploymentRecord {
            id: r.get("id"),
...
fn row_to_env(row: &sqlx::postgres::PgRow) -> DeploymentEnvironment {
    DeploymentEnvironment {
        ...
        status: row.try_get("status").ok().unwrap_or_else(|| "unknown".into()),
