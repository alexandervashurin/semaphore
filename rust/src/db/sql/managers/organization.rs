...
impl OrganizationManager for SqlStore {
    ...
    async fn update_organization(&self, id: i32, payload: OrganizationUpdate) -> Result<Organization> {
        ...
        let updated = Organization {
            ...
            quota_max_tasks_per_month: payload.quota_max_tasks_per_month.or(org.quota_max_tasks_per_month),
    ...
    async fn get_organization_users(&self, org_id: i32) -> Result<Vec<OrganizationUser>> {
        let pool = self.get_postgres_pool()?;
        let rows = sqlx::query("SELECT * FROM organization_user WHERE org_id = $1 ORDER BY created")
            .bind(org_id)
            .fetch_all(pool)
            .await
            .map_err(Error::Database)?;
        ...
        match quota_type {
            "projects" => {
                if let Some(max) = org.quota_max_projects {
                    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM project WHERE org_id = $1")
                        .bind(org_id)
                        .fetch_one(pool)
                        .await
                        .map_err(Error::Database)?;
            ...
            "users" => {
                if let Some(max) = org.quota_max_users {
                    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM organization_user WHERE org_id = $1")
                        .bind(org_id)
                        .fetch_one(pool)
                        .await
                        .map_err(Error::Database)?;
