...
use crate::models::{TaskStructuredOutput, TaskStructuredOutputCreate, TaskOutputsMap};
...
impl StructuredOutputManager for SqlStore {
    async fn get_task_structured_outputs(&self, task_id: i32, project_id: i32) -> Result<Vec<TaskStructuredOutput>> {
        let rows = sqlx::query(
    ...
    async fn get_task_outputs_map(&self, task_id: i32, project_id: i32) -> Result<TaskOutputsMap> {
        let outputs = self.get_task_structured_outputs(task_id, project_id).await?;
        let map: HashMap<_, _> = outputs.into_iter().map(|o| (o.key, o.value)).collect();
        Ok(TaskOutputsMap { task_id, outputs: map })
    }

    async fn create_task_structured_output(&self, task_id: i32, project_id: i32, payload: TaskStructuredOutputCreate) -> Result<TaskStructuredOutput> {
        let row = sqlx::query(
    ...
    async fn create_task_structured_outputs_batch(&self, task_id: i32, project_id: i32, outputs: Vec<TaskStructuredOutputCreate>) -> Result<()> {
        for output in outputs {
            self.create_task_structured_output(task_id, project_id, output).await?;
        }
        Ok(())
    }
    ...
    async fn get_template_last_outputs(&self, template_id: i32, project_id: i32) -> Result<TaskOutputsMap> {
        ...
        match last_task_id {
            ...
            None => Ok(TaskOutputsMap { task_id: 0, outputs: HashMap::new() }),
        }
    }
}
...
fn row_to_output(row: &sqlx::postgres::PgRow) -> TaskStructuredOutput {
    ...
    TaskStructuredOutput {
        ...
        key: row.get("key"),
        value,
        value_type: row.try_get("value_type").unwrap_or_else(|_| "string".into()),
