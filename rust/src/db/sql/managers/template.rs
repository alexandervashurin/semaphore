    ...
    async fn get_templates(&self, project_id: i32) -> Result<Vec<Template>> {
        ...
        Ok(rows
            ...
            .map(|row| Template {
                ...
                allow_override_args_in_task: row
                    .try_get("allow_override_args_vars")
                    .ok()
                    .unwrap_or(false),
                allow_override_branch_in_task: row
                    .try_get("allow_override_branch_in_task")
                    .ok()
                    .unwrap_or(false),
                allow_inventory_in_task: row
                    .try_get("allow_inventory_in_task")
                    .ok()
                    .unwrap_or(false),
                allow_parallel_tasks: row.try_get("allow_parallel_tasks").ok().unwrap_or(false),
                suppress_success_alerts: row
                    .try_get("suppress_success_alerts")
                    .ok()
                    .unwrap_or(false),
